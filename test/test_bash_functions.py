...
def test_IPv6_not_True_removes_ipv6(Docker, Slow, test_args, expected_ipv6, expected_stdout):
    ...
    expected_ipv6_check = (
        lambda: (
            IPV6_LINE
            in Docker.run(f"grep \'use-ipv6.pl\' {WEB_CONFIG}").stdout
        )
        == expected_ipv6
    )
...
def test_overrides_default_WEB_PORT(Docker, Slow, test_args):
    ...
    Slow(
        lambda: re.search(CONFIG_LINE, Docker.run(f'cat {WEB_CONFIG}').stdout)
        != None
    )
...
def test_overrides_default_CUSTOM_CACHE_SIZE(Docker, Slow, test_args, cache_size):
    ...
    Slow(
        lambda: re.search(
            CONFIG_LINE, Docker.run(f'cat {DNSMASQ_CONFIG}').stdout
        )
        != None
    )
...
def test_DNS_Envs_are_secondary_to_setupvars(Docker, Slow, args_env, expected_stdout, dns1, dns2):
    ...
    Docker.run(f'sed -i "/^PIHOLE_DNS/ d" {setupVars}')
    Docker.run(f'echo "PIHOLE_DNS_1={dns1}" | tee -a {setupVars}')
    if dns2:
        Docker.run(f'echo "PIHOLE_DNS_2={dns2}" | tee -a {setupVars}')
    Docker.run(f'sync {setupVars}')
    Slow(lambda: 'PIHOLE_DNS' in Docker.run(f'cat {setupVars}').stdout)
    ...
    expected_servers = [f'server={dns1}']
    if dns2:
        expected_servers.append(f'server={dns2}')
...
def test_webPassword_env_assigns_password_to_file_or_removes_if_empty(Docker, args_env, secure, setupVarsHash):
    ...
    if secure:
        ...
        assert (
            Docker.run(
                f"grep -q \'{setupVarsHash}\' /etc/pihole/setupVars.conf"
            ).rc
            == 0
        )
...
def test_webPassword_pre_existing_trumps_all_envs(Docker, args_env, test_args):
    ...
    assert (
        Docker.run(
            f"grep -q \'WEBPASSWORD=volumepass\' /etc/pihole/setupVars.conf"
        ).rc
        == 0
    )
